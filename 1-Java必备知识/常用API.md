...
- **枚举类的属性**
  ...
  - 枚举类的**使用 private final 修饰的属性应该在构造器中为其赋值**
...
public class EnumTest {
    public static void main(String[] args) {
        ...
        System.out.println(Season.SPRING);
            //com.kong.test.Season@1b6d3586
            //Season类重新ToString()后：Season{SEASONNAME='春天', SEASONDESC='春暖花开'}
        System.out.println(Season.SPRINGgetSEASONNAME());  //春天
    }
}
class Season{
    
    //声明Season对象的属性，使用private final修饰。
    public final String SEASONNAME;  //季节的名称
    public final String SEASONDESC;  //季节的描述
    ...
    //创建枚举类的实例。声明为：public static final 
...
- 使用说明
  ...
  - 使用 enum 定义的枚举类**默认继承了 java.lang.Enum类**，因此不能再继承其他类

  - 枚举类的构造器只能使用 private 权限修饰符

  - 枚举类的所有实例必须在枚举类中显式列出(**,** **分隔** **;** **结尾**)。列出的实例系统会**自动添加 public static final 修饰**，例：
...
![image-20200729210728805](常用API.assets/image-20200729210728805.png)
...
public class EnumTest {
    public static void main(String[] args) {
        ...
        //每个枚举类对象都会实现接口中的方法
        Season.SPRING.show(); //春夏秋冬
        Season.SUMMER.show(); //春夏秋冬
        Season.AUTUMN.show(); //春夏秋冬
        Season.WINTER.show(); //春夏秋冬
    }
}
...
public class EnumTest {
    public static void main(String[] args) {
        Season.SPRING.show();  //我是春天
        Season.SUMMER.show();  //我是夏天
        Season.AUTUMN.show();  //我是秋天
        Season.WINTER.show();  //我是冬天
    }
}
